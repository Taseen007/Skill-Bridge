...
const AddListing = () => {
  ...
  // Payment method input state — removed (model has no paymentMethods field)
  ...
  const validateForm = useCallback(() => {
    ...
    if (!String(formData.totalSessions).trim()) {
    ...
    } else if (Number(formData.fee) < 0 || Number(formData.fee) > 1000000) {
      newErrors.fee = "Fee must be between 0 and 1,000,000";
    }

    if (!formData.listingImgURL.trim()) {
      newErrors.listingImgURL = "Image URL is required";
    } else {
      // Accept any non-empty string — teachers may use relative paths or
      // Cloudinary URLs that don't parse with new URL(). Only reject obvious
      // non-URLs (no slashes or dots at all).
      const url = formData.listingImgURL.trim();
      const looksLikeUrl = url.startsWith("http://") || url.startsWith("https://") || url.startsWith("/");
      if (!looksLikeUrl) {
        newErrors.listingImgURL = "Please enter a valid image URL (must start with http:// or https://)";
      }
    }

    // paymentMethods are optional — the model doesn't store them and
    // they are only shown to learners after booking, not required to publish.

    if (!formData.availableSlots || formData.availableSlots.length === 0) {
      newErrors.availableSlots = "Add at least one available time slot so learners can book";
    }
  ...
  // Role guard: TeacherRoute in App.jsx already handles this, but we add a
  // component-level safety net for direct renders or stale Redux state.
        ...
        <div className="text-center p-8">
          ...
          <p className="text-gray-600 mb-6">
            {!user
              ...
              : "Only teachers can create listings. Upgrade your account in your profile to start teaching."}
          </p>
          <button
          ...
          <form onSubmit={handleSubmit} className="p-8 md:p-12">
            ...
            {currentStep === 3 && (
              <div className="space-y-8">
                ...
                {/* Available Slots — required: learners cannot book without at least one slot */}
                <div className="mt-8">
                  <label className="flex items-center gap-2 text-sm font-semibold text-gray-700 mb-1">
                    <Clock className="h-5 w-5 text-blue-600" />
                    Available One-Hour Time Slots *
                  </label>
                  {/* Warning shown until the teacher adds at least one slot */}
                  {formData.availableSlots.length === 0 && (
                    <div className="mb-3 flex items-start gap-2 bg-amber-50 border border-amber-200 rounded-lg p-3">
                      <AlertCircle className="h-4 w-4 text-amber-600 flex-shrink-0 mt-0.5" />
                      <p className="text-sm text-amber-700">
                        You must add at least one time slot. Without it, learners won't be able to book a session.
                      </p>
                    </div>
                  )}
              ...
              <div className="space-y-8">
                ...
                <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-6">
                  ...
                  <div className="space-y-3 text-sm text-yellow-700">
                    {/* Dynamic validation checklist so teacher can see exactly what's missing */}
                    {(() => {
                      const v = validateForm();
                      const allPassed = v.isValid;
                      const checks = [
                        { key: "title",         label: "Title accurately represents your offering" },
                        { key: "description",   label: "Description clearly explains what learners will get" },
                        { key: "totalSessions", label: "Total sessions is specified" },
                        { key: "skillID",       label: "A skill is selected" },
                        { key: "proficiency",   label: "Proficiency level is set" },
                        { key: "fee",           label: "Pricing is set" },
                        { key: "listingImgURL", label: "Image URL is valid" },
                        { key: "availableSlots",label: "At least one time slot is added" },
                      ];
                      return checks.map(({ key, label }) => {
                        const hasError = !!v.errors[key];
                        return (
                          <div key={key} className="flex items-start gap-3">
                            {hasError ? (
                              <svg className="h-5 w-5 text-red-500 flex-shrink-0 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                            ) : (
                              <CheckCircle className="h-5 w-5 text-green-500 flex-shrink-0 mt-0.5" />
                            )}
                            <span className={hasError ? "text-red-700 font-medium" : ""}>
                              {hasError ? v.errors[key] : label}
                            </span>
                          </div>
                        );
                      });
